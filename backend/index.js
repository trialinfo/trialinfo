...
async function update_database(connection) {
  ...
  if (!await column_exists(connection, 'marks', 'computed')) {
    console.log('Adding column `computed` to table `marks`');
    await connection.queryAsync(`
      ALTER TABLE marks
      DROP PRIMARY KEY
    `);
    await connection.queryAsync(`
      ALTER TABLE marks
      ADD COLUMN computed BOOLEAN NOT NULL
    `);
    await connection.queryAsync(`
      ALTER TABLE marks
      ADD PRIMARY KEY (id, number, round, zone, computed)
    `);
    let backtick='`';
    await connection.queryAsync(`
      UPDATE marks
      ...
      LEFT JOIN (
        SELECT DISTINCT id, number, 1 AS computed
	FROM scoring_zones
	JOIN zones USING (id, zone)
	JOIN riders USING (id, ${backtick}class${backtick})
      ) AS _1 USING (id, number)
      SET marks.computed = COALESCE(_1.computed, 0) AND scoring;
    `);
    await connection.queryAsync(`
      UPDATE events
      SET recompute = 1
  ...
  if (!await column_exists(connection, 'riders', 'computed_penalty_marks')) {
    console.log('Adding column `computed_penalty_marks` to table `riders`');
    await connection.queryAsync(`
      ALTER TABLE riders
      ADD COLUMN computed_penalty_marks FLOAT AFTER penalty_marks
    `);
  }
  if (await column_exists(connection, 'riders', 'scoring')) {
    console.log('Removing column `scoring` from table `riders`');
    await connection.queryAsync(`
      ALTER TABLE riders
      DROP COLUMN scoring
    `);
...
async function read_riders(connection, id, revalidate, number) {
  ...
  (await connection.queryAsync(`
    SELECT number, round, zone, marks, computed
    FROM marks
    WHERE ` + filters)
  ).forEach((row) => {
    let rider = riders[row.number];
    if (rider) {
      let marks = row.computed ?
        rider.computed_marks : rider.marks_per_zone;
      var round = marks[row.round - 1];
      if (!round)
	round = marks[row.round - 1] = [];
...
function compute_update_event(id, event) {
  ...
  let results = compute_event.compute_event(cached_riders, event, true);
...
async function admin_save_rider(connection, id, number, rider, tag, query) {
  ...
  try {
	...
	rider.rankings = event.rankings.map((ranking) =>
      ...
      // FIXME: Prevent updating rider.computed_marks via admin_save_rider?
...
function reset_event(base_event, base_riders, event, riders, reset) {
  if (reset == 'master' || reset == 'register' || reset == 'start') {
    Object.values(riders).forEach((rider) => {
      ...
      rider.computed_penalty_marks = null;
      rider.marks = null;
      rider.marks_per_zone = [];
      rider.computed_marks = [];
...
function result_marks_per_zone(rider, event) {
  let marks_per_zone;
  ...
  if (rider.computed_marks && rider.computed_marks.length) {
	...
	compute_event.resulting_marks_in_round(rider, round);
    }
  } else {
    marks_per_zone = rider.marks_per_zone;
  }

  let skipped_zones = (event.skipped_zones || {})[rider.class];
  if (skipped_zones) {
...
async function get_event_results(connection, id) {
  ...
  /* construct the list of included events (more than one for combined rankings) */
  for(;;) {
    ids.unshift(+id);
  ...
  /* construct a rider object based on a cached rider */
  ...
  /* the result of a rider in a particular event */
  function rider_result(rider, event) {
    let hash = {};
    for (let field of rider_result_fields)
      hash[field] = rider[field];
    if ((event.classes[rider.class - 1] || {}).non_competing)
      hash.non_competing = true;
    hash.marks_per_zone = result_marks_per_zone(rider, event);
    return hash;
  }

  function rider_starts(rider, event) {
  ...
  if (ids.length == 1) {
    for (let cached_rider of cached_riders[0]) {
      if (!rider_starts(cached_rider, last_event))
	continue;
	...
	rider[field] = cached_rider[field];
      if ((last_event.classes[class_ - 1] || {}).non_competing)
	rider.non_competing = true;
      let result = rider_result(cached_rider, events[0]);
      for (let field of ['decisive_round'])
	result[field] = cached_rider[field];
      if (cached_rider.unfinished_zones && !cached_rider.failure)
        result.rounds = cached_rider.rounds;
      rider.results.push(result);

      if (!riders_by_class[class_])
	riders_by_class[class_] = [];
      riders_by_class[class_].push(rider);
    }
  } else {
	...
	let riders = riders_by_class[class_];
	if (!riders)
	  riders = riders_by_class[class_] = {};
	...
	let result = rider_result(cached_rider, events[ev]);
    ...
    for (let class_ in riders_by_class) {
      riders_by_class[class_] =
    ...
    let results = compute_event.compute_event(all_riders, last_event, false);
...
async function get_event_stats(connection, id) {
  ...
  for (let rider of Object.values(riders)) {
    let rc = ranking_class(rider, event); /* XXX */
    if (rc == null)
      continue;
    let marks_per_zone = result_marks_per_zone(rider, event);
...
async function get_section_lists(connection, id) {
  ...
  function base_rider(rider, event) {
    let hash = {
    };
  ...
  for (let rider of Object.values(riders)) {
    ...
    riders_by_ranking_class[rc].push(base_rider(rider, event));
  }
...
async function __update_rider(connection, id, number, old_rider, new_rider) {
  ...
  for (let field of ['marks_per_zone', 'computed_marks']) {
    let computed = (field == 'computed_marks');
    await zipAsync(old_rider[field], new_rider[field],
...
async function register_get_event(connection, id, user) {
  ...
  event.classes.forEach((class_, index) => {
	...
	name: class_.name,
	ranking_class: class_.ranking_class /* XXX */
      };
    }
  });
  return result;
...
function serie_index(view) {
  return async function(req, res, next) {
	...
	FROM series_events
	JOIN series_classes USING (serie)
	JOIN classes USING (id, ranking_class)`)) /* XXX */
      .forEach((se) => {
...
async function scoring_get_info(connection, id) {
  ...
  for (let c in classes) {
    let class_ = {};
    for (let field of ['rounds', 'name', 'color', 'ranking_class', 'time_limit']) /* XXX */
      class_[field] = event.classes[c - 1][field];
    hash.event.classes[c - 1] = class_;
...
function scoring_update_riders(id, event, riders) {
  ...
  function scoring_update_rider(rider) {
    ...
    let computed_marks = [];
    let computed_penalty_marks = null;
	...
	continue;
      let marks_per_round = computed_marks[item.round - 1];
      if (!marks_per_round) {
	marks_per_round = [];
	computed_marks[item.round - 1] = marks_per_round;
      }
      marks_per_round[item.zone - 1] = item.marks;
      if (item.penalty_marks)
	computed_penalty_marks += item.penalty_marks;
    }

    if (!deepEqual(rider.computed_marks, computed_marks) ||
        rider.computed_penalty_marks != computed_penalty_marks) {
      Object.assign(cache.modify_rider(id, rider.number), {
	computed_marks: computed_marks,
	computed_penalty_marks: computed_penalty_marks
      });
    }
  }
