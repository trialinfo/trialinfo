...
@media print {\n\
  ...
  $scope.total_marks = function(rider) {
    return rider.marks == null ? null :
      score(rider.marks, event.split_score);
  }

  $scope.additional_marks = function(rider) {
    return score(rider.additional_marks, event.split_score) || '';
  }

  $scope.score = function(rider) {
    var score_;
    try {
      score_ = rider.rankings[$scope.show.ranking - 1].score;
    } catch(_) { }
    return score(score_, event.split_score);
  }
